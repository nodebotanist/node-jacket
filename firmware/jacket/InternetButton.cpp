...
InternetButton_NeoPixel ring = InternetButton_NeoPixel(PIXEL_COUNT, pin, PIXEL_TYPE);
...
void InternetButton::rainbow(uint8_t wait) {
  ...
  for(j=0; j<256; j++) { // 1 cycle of all colors on wheel
    for(i=0; i< 12; i++) {
      uint8_t WheelPos = ((i * 256 / ring.num_pixels()) + j) & 255;
      if(WheelPos < 85) {
...
/*-------------------------------------------------------------------------
  ...
  STM32_Pin_Info* IB_PIN_MAP_2 = HAL_Pin_Map(); // Pointer required for highest access speed
  #define pinLO(_pin) (IB_PIN_MAP_2[_pin].gpio_peripheral->BSRRH = IB_PIN_MAP_2[_pin].gpio_pin)
  #define pinHI(_pin) (IB_PIN_MAP_2[_pin].gpio_peripheral->BSRRL = IB_PIN_MAP_2[_pin].gpio_pin)
#else
...
InternetButton_NeoPixel::InternetButton_NeoPixel(uint16_t n, uint8_t p, uint8_t t) :
  numLEDs(n), numBytes(n*3), brightness(0), ib_pixels(NULL), type(t), endTime(0)
{
  if((ib_pixels = (uint8_t *)malloc(numBytes))) {
    memset(ib_pixels, 0, numBytes);
  }
}

InternetButton_NeoPixel::~InternetButton_NeoPixel() {
  if(ib_pixels) free(ib_pixels);
  pinMode(pin, INPUT);
}

void InternetButton_NeoPixel::begin(void) {
  pinMode(pin, OUTPUT);
  digitalWrite(pin, LOW);
}

void InternetButton_NeoPixel::show(void) {
  if(!ib_pixels) return;
   ...
   *ptr = ib_pixels,   // Pointer to next byte
...
void InternetButton_NeoPixel::setPin(uint8_t p) {
  pinMode(pin, INPUT);
  pin = p;
  pinMode(p, OUTPUT);
  digitalWrite(p, LOW);
...
void InternetButton_NeoPixel::setPixelColor(
...
void InternetButton_NeoPixel::setPixelColor(uint16_t n, uint32_t c) {
  if(n < numLEDs) {
    uint8_t
      r = (uint8_t)(c >> 16),
      g = (uint8_t)(c >>  8),
...
uint32_t InternetButton_NeoPixel::Color(uint8_t r, uint8_t g, uint8_t b) {
  return ((uint32_t)r << 16) | ((uint32_t)g <<  8) | b;
}

// Query color from previously-set pixel (returns packed 32-bit RGB value)
uint32_t InternetButton_NeoPixel::getPixelColor(uint16_t n) const {
  if(n >= numLEDs) {
...
uint8_t *InternetButton_NeoPixel::get_pixels(void) const {
  return ib_pixels;
}

uint16_t InternetButton_NeoPixel::num_pixels(void) const {
  return numLEDs;
}
...
void InternetButton_NeoPixel::setBrightness(uint8_t b) {
...
uint8_t InternetButton_NeoPixel::getBrightness(void) const {
  return brightness - 1;
}

void InternetButton_NeoPixel::clear(void) {
  memset(ib_pixels, 0, numBytes);
}
