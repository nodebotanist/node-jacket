...
class InternetButton_NeoPixel {

 public:

  // Constructor: number of LEDs, pin number, LED type
  InternetButton_NeoPixel(uint16_t n, uint8_t p=2, uint8_t t=WS2812B);
  ~InternetButton_NeoPixel();
 ...
 private:
  ...
  uint8_t
    ...
    brightness,
   *ib_pixels;        // Holds LED color values (3 bytes each)
  uint32_t
    endTime;       // Latch timing reference
};

#endif // InternetButton_NEOPIXEL_H
